...
void nicify_diffs_more(T_t &T, typename boost::graph_traits<T_t>::vertex_descriptor t)
{
  ...
  if (t_size <= c0_size + 1 && t_size + 1 >= c0_size)
    {  
      ...
      decltype(T[t].bag) diff_inst;
      if (t_size < c0_size)
        std::set_difference(T[c0].bag.begin(), T[c0].bag.end(), T[t].bag.begin(), T[t].bag.end(), std::inserter(diff_inst, diff_inst.end()));
      else
        std::set_difference(T[t].bag.begin(), T[t].bag.end(), T[c0].bag.begin(), T[c0].bag.end(), std::inserter(diff_inst, diff_inst.end()));
      wassert(diff_inst.size());
