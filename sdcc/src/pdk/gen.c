...
// Todo: Stack offsets might be unavailable during dry run (messes with addition costs, so we should have a mechanism to do it better).
  int min = TARGET_IS_PDK13 ? -16 : (TARGET_IS_PDK14 ? -32 : -64);

  return (aop->aopu.bytes[offset].byteu.stk - G.stack.pushed >= min || regalloc_dry_run);
}
  ...
  for (int i = up ? 0 : size - 1; up ? i < size : i >= 0; up ? i++ : i--)
    {
      ...
      if ((IDXSP || SPRELMODE) && (s + i - G.stack.pushed >= min))
        {
      ...
      if ((IDXSP || SPRELMODE) && (d + i - G.stack.pushed >= min))
        {
...
genNot (const iCode *ic)
{
  ...
  if (IS_BOOL (operandType (left)))
    {
      if ((aopInReg (left->aop, 0, P_IDX) || left->aop->type == AOP_DIR || SPRELMODE && aopOnStackNotExt (left->aop, 0, 1)) && aopSame (result->aop, 0, left->aop, 0, 1))
        {
          emit2 ("mov", "a, #0x01");
...
genCpl (const iCode *ic)
{
  ...
  if (result->aop->type == AOP_STK && !(aopOnStackNotExt (result->aop, 0, size) && SPRELMODE))
    {
      bool pushed_p = false;
  ...
  for (int i = 0; i < size; i++)
    {
      ...
      if ((left_aop->type == AOP_DIR || aopInReg (left_aop, i, P_IDX) || SPRELMODE && aopOnStackNotExt (left_aop, i, 1)) && right_aop->type != AOP_STK && aopSame (left_aop, i, result_aop, i, 1))
      ...
      else if (aopInReg (right_aop, P_IDX, i) && left_aop->type == AOP_STK && !((IDXSP || SPRELMODE) && aopOnStackNotExt (left_aop, i, 1)))
        {
        ...
        (left_aop->type == AOP_LIT || left_aop->type == AOP_IMMD || left_aop->type == AOP_DIR || aopInReg (left_aop, i, P_IDX) || SPRELMODE && aopOnStackNotExt (left_aop, i, 1)))
        {
          if ((TARGET_IS_PDK15 || TARGET_IS_PDK16) && (left_aop->type == AOP_IMMD || aopInReg (left_aop, i, P_IDX) || SPRELMODE && aopOnStackNotExt (left_aop, i, 1)))
            {
      ...
      else if (right_aop->type == AOP_STK && !(SPRELMODE && aopOnStackNotExt (right_aop, i, 1)) || right_aop->type == AOP_STL && !i)
        {
      ...
      if (i + 1 < size && (result_aop->type == AOP_STK && !(SPRELMODE && aopOnStackNotExt (result_aop, i, 1))) && (aopInReg (left_aop, i + 1, P_IDX) || aopInReg (right_aop, i + 1, P_IDX))) // Avoid overwriting still-needed p when storing onto stack.
        {
          emit2 ("xch", "a, p");
           G.p.type = AOP_INVALID;
          pushAF ();
          ...
          for (int i = 0; i < left->aop->size; i++)
            {
              cheapMove (ASMOP_A, 0, left->aop, i, true, true, true);
              pushAF ();
              if ((IDXSP || SPRELMODE) && -4 - G.stack.pushed >= min)
                {
  ...
  for (int i = 0; i < size; i++)
    {
      ...
      else if (right->aop->type == AOP_STK && !(SPRELMODE && aopOnStackNotExt (right->aop, i, 1)) || right->aop->type == AOP_STL && !i)
        {
  ...
  if (!sign && size == 1 && ifx && aopInReg (left->aop, 0, A_IDX))
    {
      ...
      else if (ic->op == '<' && aopInReg (left->aop, 0, A_IDX) && (right->aop->type == AOP_LIT || right->aop->type == AOP_DIR || aopInReg (right->aop, 0, P_IDX) || SPRELMODE && aopOnStackNotExt (right->aop, 0, 1)) && (IC_TRUE (ifx) || !regDead (A_IDX, ic)))
        {
          if (IC_FALSE (ifx))
            {
  ...
  for (int i = 0; i < size; i++)
    {
      ...
      if (!started && aopIsLitVal (right->aop, i, 1, 0x00) && i + 1 < size)
        ...
        {
          if (left->aop->type == AOP_STK && !((IDXSP || SPRELMODE) && aopOnStackNotExt (left->aop, i, 1)))
            {
              cheapMove (ASMOP_A, 0, left->aop, i, true, true, !i);
...
genCmpEQorNE (const iCode *ic, iCode *ifx)
{
  ...
  if (aopInReg (left->aop, 1, A_IDX) && (right->aop->type == AOP_LIT || right->aop->type == AOP_DIR || right->aop->type == AOP_IMMD))
    ...
    for (int i = 0; i < size; i++)
      {
        if (left->aop->type == AOP_STK && !(SPRELMODE && aopOnStackNotExt (left->aop, i, 1)) && aopInReg (right->aop, i, P_IDX))
          {
        ...
        else
          {
            ...
            cheapMove (ASMOP_A, 0, left->aop, i, true, true, true);
            if (right->aop->type == AOP_SFR || right->aop->type == AOP_STK && !(SPRELMODE && aopOnStackNotExt (right->aop, i, 1)) || right->aop->type == AOP_CODE || right->aop->type == AOP_STL && !i)
              {
        ...
        if (ifx && i + 1 == size && ((ic->op == EQ_OP) ^ (bool)(IC_FALSE(ifx))))
          {
            if (TARGET_IS_PDK13)
              {
                symbol *tlbl = (regalloc_dry_run ? 0 : newiTempLabel (NULL));
                emit2 ("ceqsn", "a, %s", (right->aop->type == AOP_SFR || right->aop->type == AOP_STK && !(SPRELMODE && aopOnStackNotExt (right->aop, i, 1)) || right->aop->type == AOP_CODE || right->aop->type == AOP_STL && !i) ? "p" : aopGet (right->aop, i));
                emitJP (tlbl, 0.0f);
                cost (2, 3);
                emitJP (IC_FALSE (ifx) ? IC_FALSE (ifx) : IC_TRUE (ifx), 0.0f);
                emitLabel (tlbl);
              ...
              {
                emit2 ("cneqsn", "a, %s", (right->aop->type == AOP_SFR || right->aop->type == AOP_STK && !(SPRELMODE && aopOnStackNotExt (right->aop, i, 1)) || right->aop->type == AOP_CODE || right->aop->type == AOP_STL && !i) ? "p" : aopGet (right->aop, i));
                cost (1, 1);
                emitJP (IC_FALSE (ifx) ? IC_FALSE (ifx) : IC_TRUE (ifx), 0.0f);
              }
          }
      ...
      for (int i = 0; i < right->aop->size; i++)
        {
          ...
          else if ((left->aop->type == AOP_DIR || SPRELMODE && aopOnStackNotExt (right->aop, i, right->aop->size - i)) && right->aop->type == AOP_LIT) // Try to combine multiple bytes with 0xff mask by or.
            {
              int j, k;
  ...
  if (right->aop->type == AOP_LIT)
    {
      ...
      if (result->aop->type == AOP_STK && !(SPRELMODE && aopOnStackNotExt (result->aop, 0, size)) && (!regDead (A_IDX, ic) || loop))
        pushAF ();
  ...
  else
    {
      ...
      for(int i = 0; i < size; i++)
        {
          if (result->aop->type == AOP_STK && !(SPRELMODE && aopOnStackNotExt (result->aop, i, 1)) && size == 1)
            {
              pushAF ();
          ...
          else if (result->aop->type == AOP_STK && !(SPRELMODE && aopOnStackNotExt (result->aop, i, 1)))
            {
              ...
              if (aopInReg (result->aop, size - 1, A_IDX) &&
              ...
              else
                {
                  if (size > 1 && aopInReg (result->aop, 0, A_IDX) || !regDead (A_IDX, ic))
                   ...
                   emit2 ("sl", "a");
                   cost (1, 1);
                   if (result->aop->type == AOP_STK && !(SPRELMODE && aopOnStackNotExt (result->aop, size - 1, 1)))
                     {
...
#endif
  if (right->aop->type == AOP_STK && !((IDXSP || SPRELMODE) && aopOnStackNotExt (right->aop, 0, size)) && !bit_field && aopInReg (left->aop, 0, A_IDX))
    {
      if (!regDead (P_IDX, ic))
        {
          pushPF (false);
...
genIfx (const iCode *ic)
{
  ...
  if(cond->aop->type == AOP_STK && !(SPRELMODE && aopOnStackNotExt (cond->aop, 0, cond->aop->size)) && !regDead (P_IDX, ic))
    {
      cost (200, 200);
      wassert (regalloc_dry_run);
    }
      ...
      for (offset = 0; offset < size; offset++)
        {
          if (offset == skipbyte)
            continue;
          if (right->aop->type == AOP_STK && !(SPRELMODE && aopOnStackNotExt (right->aop, offset, 1)))
            {
