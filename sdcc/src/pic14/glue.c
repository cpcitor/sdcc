...
pic14_allocateSpace (symbol *sym, int size)
{
  ...
  if (!pic14_stringInSet(sym->rname, &emitted, TRUE))
    {
      DBG_MSG("global definition UD udata: module %s section %u name %s size %d",
              moduleName, sectionNr, sym->rname, size);
...
addToAbsMap (symbol *sym)
{
  ...
  int addr;
      ...
      if (IS_DEFINED_HERE(sym) && SPEC_ABSA(sym->etype))
      {
        ...
        if (absmap_max == -1 || addr > absmap_max) absmap_max = addr;
        if (absmap_min == -1 || addr < absmap_min) absmap_min = addr;
...
pic14_constructAbsMap (struct dbuf_s *oBuf)
{
  ...
  for (addr=absmap_min; addr <= absmap_max; addr++)
  {
    ...
    if (aliases && elementsInSet(aliases)) {
      ...
      DBG_MSG("global definition UD_abs udata_ovr module %s address 0x%04X size %d",
          moduleName, addr, size);
        ...
        ovrset = setNextItem (ovrSetSets))
        {
                ...
                sym = setNextItem (ovrset))
                {
                        ...
                        if ((sym->_isparm && !IS_REGPARM (sym->etype))
                                && !IS_STATIC (sym->etype) && !IS_STATIC (sym->localof->etype))
                                addToPublics (sym);
...
pic14_printIvalLabel(struct dbuf_s *oBuf, symbol *sym)
{
    ...
    if (SPEC_ABSA(sym->etype))
      ...
      {
        DBG_MSG("printIvalLabel %s %s module %s section %u name %s",
            label, segname, moduleName, sectionNr, sym->rname);
      ...
      if (in_code)
      ...
      else
        {
          dbuf_printf (oBuf, "\tdb\t0x%02x, 0x%02x, 0x%02x, 0x%02x\n",
              (unsigned int)(*s & 0xff),
              (unsigned int)((*s >> 8) & 0xff),
              (unsigned int)((*s >> 16) & 0xff),
              (unsigned int)((*s >> 24) & 0xff));
        }
    }
}
...
/*-----------------------------------------------------------------------*/
static int
pic14_printIvalChar16 (symbol * sym, sym_link * type, initList * ilist, struct dbuf_s *oBuf, const TYPE_TARGET_UINT *s, bool check)
{
  value *val;
  size_t size = DCL_ELEM(type);
  TYPE_TARGET_UINT *p;
...
//    inInitMode++;
    ...
    for (nm = namedspacemaps; nm; nm = nm->next)
    {
        DBG_MSG("named space map %p sname '%s' dbName '%c' is_const %u: %d symbols",
            nm, nm->map->sname, nm->map->dbName, nm->is_const,
            nm->map->syms ? elementsInSet(nm->map->syms) : 0);
        if (nm->is_const)
